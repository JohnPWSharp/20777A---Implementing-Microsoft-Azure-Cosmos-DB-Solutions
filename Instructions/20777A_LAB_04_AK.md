...
6. In File Explorer, in the **E:\\Labfiles\\Lab04\\Starter** folder, right-click **Setup.cmd**, and then click **Run as administrator**.
7. When prompted enter your Azure credentials.
8. On the toolbar, click **Internet Explorer**.
9. In Internet Explorer, go to **http://portal.azure.com**, and sign in using the Microsoft account that is associated with your Azure Learning Pass subscription.
10. In the Azure portal, in the left panel, click **All resources**, and then click **20777-mod4-sql-&lt;your initials and day&gt;**.
11. On the **20777-mod7-sql-&lt;your initials and day&gt;** blade, under **SETTINGS**, click **Keys**.
...
3. In the toolbar, click the ellipsis (**...**), and then click **Settings**.
4. On the **Settings** blade, under **Page options**, click **Custom**.
5. In the **Query results per page** box, type **600**, and then click **Save**.
...
9.  Click **Query Information**, and make a note of the **Request Charge** showing the number of RUs required to perform this query.
...
12. Click **Query Information**, and compare the **Request Charge** to that of the previous query. It should be significantly lower.
...
14. Click **Results**. This query should return the details of a single product from a **ProductWithPriceHistory** document.
15. Click **Query Information**, and note the **Request Charge**.
...
18. Click **Query Information**, and compare the **Request Charge** to that of the previous query. It should be lower than the previous query. This is because the amount of I/O that Cosmos DB had to perform to fetch the document is much lower due to the document being smaller.
...
1. In Internet Explorer, in the left panel, click **All resources**, and then click **20777-mod4-sql-&lt;your initials and day&gt;**.
...
5. In File Explorer, go to **E:\\Labfiles\\Lab04\\Starter**, right-click **create-database-partitioned-by-subcategory.ps1**, and then click **Edit**.
...
11. In the **Open** dialog box, in the **E:\\Labfiles\\Lab04\\Starter** folder, click **create-database-partitioned-by-productid.ps1**, and then click **Open**.
...
13. In the **Data + Index storage consumed by top partition keys** graph, note that this time, the partitons are very skewed, with the vast majority of documents in the **ProductHOstory** partition. Rememmber from exercise 1 that a new  **ProductHistory** document is created every time a product has a price change, so this skew is unlikely to change and will probably only get worse.
...
19. Set the **Start**, and **End** times to the time you noted earlier when running the application.
...
8.  Locate the **SaveDocumentWithoutUsingTrigger** method. This code saves the **Product** document without firing the trigger. It then creates a **ProductHistory** document and inserts it into the database. The request charge for both operations is aggregated and returned.
