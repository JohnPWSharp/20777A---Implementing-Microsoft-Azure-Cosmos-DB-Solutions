...
2.  Delete the collection, and recreate it withou a unique index.
3.  Add the three documents that you inserted during Task 3.
4.  Query the collection to verify that all three documents are present.
5.  Use the **Scale & Settings** blade set the Time to Live (TTL) for the Customers collection to 10 seconds.
6.  Save the changes, wait for 10 seconds, and then query the collection again. Note that all the previous documents have been deleted.
7.  Reset the TTL for the Customers collection to "Off".
