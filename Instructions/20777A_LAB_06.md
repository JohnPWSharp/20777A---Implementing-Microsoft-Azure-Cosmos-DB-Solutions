...
1. Using File Explorer, navigate to **E:\\Labfiles\\Lab06\\Starter**, run **Setup.cmd** as **Administrator**.
2. When prompted enter your Azure credentials.
3. On the toolbar, click **Internet Explorer**.
4.   In Internet Explorer, go to **http://portal.azure.com**, and sign in using the Microsoft account that is associated with your Azure Learning Pass subscription.
5.   On the left, click **All resources**, and then click **20777a-mod6-sql-&lt;your initials and day&gt;**.
6.   On the **20777a-mod6-sql-&lt;your initials and day&gt;** blade, click **Keys**. 
7.   Make a note of the **URI**, and **PRIMARY KEY** values.
8.   On the left, click **All resources**, and then click **20777blobmod6&lt;your initials and day&gt;**.
9.   Under **Settings**, click **Access keys**.
10.  Make a note of the **Connect string** under **key1**.
11.  On the Start menu, click **Visual Studio 2017**.
12.  On the File menu, point to **Open**, and then click **Project/Solution**.
13.  In the **Open Project** dialog box, go to **E:\\Labfiles\\Lab06\\Starter\\Exercise01\\MigrateProductData**, click **MigrateProductData.sln**, and then click **Open**.
14. In Solution Explorer, double-click **App.config**.
15. In the **Value** attribute of the **BlobStorageConnectionString** key, paste the value you noted in step 10, replacing the text **enter Blob storage connection string**.
16. Press F5 to build and run the application. 
17. In the console window, once completed, press Enter.
...
3. Modify the **Indexing Policy** to include the excluded paths shown below, and then click **Save**. This policy prevents the **description**, **documentation**, and **imaages** fields from being indexed. The **description** field is free text and can be quite large; you will configure Azure Search to index this field more efficiently. The **documentation** and **images** fields are references to items in blob storage, and indexing these fields is not worthwhile.
...
14. Mark the following fields as **Searchable**:
    ...
    - **model**
    - **description**
15. Select **Suggester**, set the **suggester name** to **suggest**, and set the **Search mode** to **analyzeInfixMatching**.
...
26. Go back to the **Product Catalog** page and search for **Bib-Shorts**. The results should list three products (Men's Bib-Shorts, S, M and L).
...
10. Inside the &lt:script&gt; tag, after the comment **TODO: Replace anything that looks like HTML markup from the SearchString field after performing autocomplete**, add the following JavaScript function:
...
12. In the **Search** box, type **nat**. A list of suggestions should appear, with the matching text highlighted in bold. Note that similar items are included that are not necessarily an exact match for the text; this is because the repository uses fuzzy matching with the suggester.
13. Change the text to **natu**, and the list should be refined to two entries.
14. Click one of the entries and then click **Search**. The result should be two products.
15. Click **Details**, and you should see that both products have identical descriptions containing the word **natural**.
16. Clear the **Search** box, and then enter **bib**. Select **Men's Bib-Shorts, M** and then click **Search**. This time only a single product should be displayed.
...
1. Using Visual Studio, open **GenerateOrderData** solution in the **E:\Labfiles\Lab 6\Data\GenerateOrderData** folder.
...
2. Run the following command. This command uses the pscp utility to upload the jar file containing the Cosmos DB connector for Spark to the first head node of your HDInsight cluster. Enter **Pa55w.rdPa55w.rd** when prompted for a password:
...
3. Switch to the PuTTY window and run the following command. Verify that the file **azure-cosmosdb-spark_2.3.0_2.11-1.2.0-uber.jar** is listed:

    ```Bash
    ls
    ```

4. Run the following command to copy the Cosmos DB connector to the folder required by Spark applications:

    ```Bash
    sudo cp **.jar /usr/hdp/current/spark2-client/jars/
    ```

5. Run the following commands to copy the Cosmos DB connector to the same folder on the remaining head node and worker node of the HDInsight cluster. At each prompt **Are you sure you want to continue connecting**, type **yes**, and enter the pasword **Pa55w.rdPa55w.rd**:
    ...
    > **Note:** The attempt to connect to the second worker node, dnoted by the $wn1 variable, might cause the error “Name or service not known”. This is because sometime the worker nodes are suffixed wn0 and wn1, and sometimes wn0 and wn2 when they are created by HDInsight. If this occurs, change all references to the text **wn1** to **wn2** and repeat the failing commands.

6. Return to the Azure portal and go to your HDInsight cluster.
7. On the **HDInsight cluster** blade, in the **Quick Links** section, click **Cluster dashboard**.
8. Log in as **sparkadmin** with password **Pa55w.rdPa55w.rd** when prompted.
9. On the **Cluster dashboards** blade, click **Jupyter Notebook**. Internet Explorer opens a new page connected to the Jupyter UI.
10. On the **Jupyter** page, click **New** and then click **Spark**. This action creates a Scala notebook that can perform Spark queries.
11. In the page heading, click **Untitled** and change the name of the notebook to **Monthly Analysis**.
    ...
    In the first cell of the notebook, add the following Scala code. This code creates a useful utility class that can convert between datetime values and ticks (as used by the documents in the CosmosDB database):
...
12. On the **Insert** menu, click **Insert Cell Below**.
13. In the new cell, add the following code
...
14. On the **Insert** menu, click **Insert Cell Below**.
15. In the new cell, add the following code. This code imports the libraries required by the Cosmos DB connector:
...
16. On the **Insert** menu, click **Insert Cell Below**.
17. In the new cell, add the following code. This uses an %%sql magic to drop the orders table in the Hive database if it currently exists. This step is nessary as you will create a virtual table, named orders, that references the orders in the Data collection in the CosmosDB database:
...
21. In the new cell, add the following code. This drops the orderitems table in the Hive database. You will create another table that contains the order items from all of the orders:
...
22. On the **Insert** menu, click **Insert Cell Below**.
...
24. On the **Insert** menu, click **Insert Cell Below**.
25. Add the following Scala code to the new cell. This code creates a Spark dataframe that retrieves all documents from the CosmosDB collection (via the orders virtual table) for orders that have been placed in the last month:
...
26. On the **Insert** menu, click **Insert Cell Below**.
27. Add the following  Scala code to the new cell. This code creates a new dataframe that summarizes the data just fetched, to generate a count of the number of orders in the last month, grouped by order status:
...
32. On the **Insert** menu, click **Insert Cell Below**.
33. Add the following Scala code to the new cell. This code sorts the order item summary records by number ordered. The result shows the products in decreasing order or popularity:
...
34. On the **Insert** menu, click **Insert Cell Below**.
35. Add the following Scala code to the new cell. This code sorts the order item summary records by revenue:
...
36. On the **Insert** menu, click **Insert Cell Below**.
37. Add the following Scala code to the new cell. This code configures a connection that you will use to write the summary data just generated back to the Cosmos DB database. Replace **\~URI\~** and **\~KEY\~** with the values for your Cosmos DB account:
...
38. On the **Insert** menu, click **Insert Cell Below**.
39. Add the following Scala code to the new cell. This code creates a text value containing the month and year. You will add this value to the summary data:
...
40. On the **Insert** menu, click **Insert Cell Below**.
...
42. On the **Insert** menu, click **Insert Cell Below**.
43. Add the following Scala code to the new cell. This code is similar to the previous cell except that it saves the data in the "revenues by status" dataframe:
...
44. On the **Insert** menu, click **Insert Cell Below**.
45. Add the following Scala code to the new cell. This code saves the dataframe that shows the popularity of products:
...
46. On the **Insert** menu, click **Insert Cell Below**.
47. Add the following Scala code to the new cell. This code displays a dataframe that shows the revenue generated by product. You do not need to save this data,a s it is the same as that generated by te previous dataframe, just in a different order:
...
48. In the toolbar, click the **Save and checkpoint** icon.
49. On the **Cell** menu, click **Run All**. You should see the results output by each cell as it is executed.
...
50. When the notebook has completed, using the Azure portal go to your Cosmos DB account, select **Document Explorer**, select the **Adventure-Works** database, select the **Data** collection, and then click **New SQL Query**. Enter the following query, and then click **Execure Query**:

    ```SQL
    SELECT ** FROM c WHERE c.doctype = "MonthlySummary"
    ```

51. Verify that a set of summary documents appear. The example below shows the typical documents showing the number of orders by order status:
...
5. In the toolbar to the left of the **pydocumentdb** blade, click **Azure Databricks**.
6. On the **Azure Databricks** blade, under **Common Tasks** click **New Notebook**.
7. In the **Create Notebook** dialog box, in the **Name** box, type **Analyze Orders**, set the **Language** to **Python**, and then click **Create**
...
15. Add another cell and enter the following code. This code creates a Spark dataframe from the rows in the **orderdata** table, and displays the first 20 rows.
...
16. Run the cell.
17. Add another cell and enter the following code. This code creates another Spark dataframe that summarizes the volume of sales and revenue generated for each product.
...
18. Run the cell.
19. Add another cell and enter the following code. These statements create a further dataframe that sorts the data by volume ordered and displays the results.
...
20. Run the cell. The results should appear in a tabular format, with the most popular product at the top.
21. Click the graph icon below the table and select **Bar**, and then click **Plot Options**.
22. In the **Customize Plot** window, remove the fields from the **Keys** and **Values** boxes, drag the **productname** field to the **Series groupings** box, drag the **numberordered** field to the **Values** box, set **Aggregation** to **SUM**, and then click **Apply**. The bar chart should be displayed in the notebook.
23. Add another cell and enter the following code. These statements create a final dataframe that sorts the data by vrevenue and displays the results.
...
24. Run the cell.
25. Click the graph icon below the table and select **Bar**, and then click **Plot Options**.
26. In the **Customize Plot** window, remove the fields from the **Keys** box (leave the **Values** box set to **Revenue**), drag the **productname** field to the **Series groupings** box, set **Aggregation** to **SUM**, and then click **Apply**.

### Task 4: Create a Dashboard

1. Return to the cell showing the bar chart that sorts orders by product volume.
2. In the toolbar at the top right-hand side of the final cell in the notebook, click the **Show in Dashboard** icon (this icon has the image of a graph). Click **Add to New Dshboard**.
