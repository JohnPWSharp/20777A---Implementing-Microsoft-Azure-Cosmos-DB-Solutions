...
- [Lab Answer Key: Module 1: Introduction to Azure Cosmos DB](#lab-answer-key-module-1-introduction-to-azure-cosmos-db)
    - [Lab: Introduction to Azure Cosmos DB](#lab-introduction-to-azure-cosmos-db)
        - [Exercise 1: Create an Azure Cosmos DB database with a SQL API](#exercise-1-create-an-azure-cosmos-db-database-with-a-sql-api)
            - [Task 1: Prepare the environment](#task-0-prepare-the-environment)
            - [Task 2: Create a document database using the Cosmos DB SQL API](#task-1-create-a-document-database-using-the-cosmos-db-sql-api)
            - [Task 3: Add customer documents to a collection](#task-2-add-customer-documents-to-a-collection)
            - [Task 4: Perform SQL queries](#task-3-perform-sql-queries)
            - [Task 5: Configure document lifetime](#task-4-configure-a-documents-lifetime)
            - [Task 6: Replicate the Cosmos DB Account across regions](#task-5-replicate-the-cosmos-db-account-across-regions)
...
9. In the **Location** drop-down list, click the region closest to your current location, and then click **Create**.
10. Wait for the Azure Cosmos DB to be created—this might take a few minutes.
11. On the **Azure Cosmos DB** blade, **Refresh**, and then click **20777a-sql-\<your name\>-\<the day\>**.
...
18. In the **Throughput** box, change **1000** to **5000**.
19. Under **Unique keys**, click **+ Add unique key**.
20. In the text box, type **/customerid**, and then click **OK**.
