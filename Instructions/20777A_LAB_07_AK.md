...
23. Replace **~CONNECTION STRING~** with the **key1 Connection string** you noted earlier for the **20777blobmod7&lt;your initials and day&gt;** Blob storage account.
24. Press F5 to build and run the application. 
25. In the console window, wait until the script has completed.
26. On the **File** menu, click **Exit**.

### Task 2: Create a Cosmos DB collection for holding product sales statistics

1. In Internet Explorer, in the left panel, click **All resources**, and then click **20777-mod7-sql-&lt;your initials and day&gt;**.
...
1. In the left panel, click **+ Create a resource**, in the search box, type **Azure Databricks**, and then press Enter.
...
4. Under **Resource group** box, click **Use existing**, and then in the drop-down list, click **20777Mod07**.
...
8. On the **Cosms DB Connector** page, in the **&lt;your name&gt;-cluster** row, select the **Attach** check box, and wait until the status changes to **Attached**.
...
12. Click any document in the collection. It should contain data similar to that shown below. The **timeslot** field indicates the end of the 30 second period during which the data was captured. The **numberordered** value shows how many items of the specified product were ordered in that interval, and the **numberonbackorde** indicates how many items were placed on backorder because there is insufficient stock in place to satisfy orders.
...
1. In Internet Explorer, in the left panel, click **+ Create a resource**.
...
6. Under **Resource group**, click **Use existing**, then in the drop-down list, click **20777Mod07**.
7. In the **Location** box, select a location near you, and then click **Create**.
8.  In the left panel, click **All resources**, and then click **sb-20777-&lt;your name&gt;-&lt;the day&gt;**.
...
4. On the **New Function** blade, in the **Language** drop-down list, click **C#**.
5. In the **Name** box, type **HandleOrders**.
6. Under **Azure Cosmos DB trigger**, click **new**.
7. In the **Connection** dialog box, click **Azure Cosmos DB account**.
8. In the **Subscription** drop-down list, select your own subscription.
9. In the **Database Account** drop-down list, click **20777-mod7-sql-&lt;your initials and day&gt;**, and then click **Select**.
10. On the **New Function** blade, in the **Collection name** box, type **Data**.
11. Select the **Create lease collection if it does not exist** check box.
12. In the **Database name** box, type **Adventure-Works**.
13. Leave **Collection name for leases** set to **leases**, and then click **Create**.
14. On the **run.csx** blade, type the following code to add references to the **Microsoft.ServiceBus** and **Newtonsoft.Json** assemblies at the top of the file, after the reference to the **Microsoft.Azure.Documents.Client** assembly, as shown below:
...
15. Type the following code to add **Using** directives that bring the **Microsoft.ServiceBus.Messaging** and the **Newtonsoft.Json** namespaces into scope, as shown below:
...
16. After the closing brace of the **Run** function, add the following class definitions to the file:
...
17. Modify the definition of the **Run** method to indicate that it supports asynchronous operations.

    ```CSharp
    public static async Task Run(IReadOnlyList<Document> documents, TraceWriter log)
    ```

18. Remove the following two statements from the body of the function:
...
19. Inside the **if** statement, add the following two lines of code that create client connections for each of the ServiceBus message queues that you created earlier. Replace the **&lt;Connection String&gt;** with the **Primary Connection String** value that you noted earlier for the **sb-20777-&lt;your name&gt;-&lt;the day&gt;** Service Bus.
    ...
    ```
20. Still inside the **if** statement, immediately after the code you have just entered, add the following statements.
...
21. Inside the **foreach** statement, add the following code:
...
22. Inside the nest **if** statement you added in the previous step, add the following code:
...
23. Before the closing brace of the outermost **if** statement, add the following code:
...
8.  In Internet Explorer, click **Search By Category**, for the first item returned by the search, click **Details**, and then click **Add to Shopping Cart**.
...
22. Press F5 to build and run the application. You should see a message appear that describes the order you just placed using the Adventure-Works web site (if you placed more than one order, you will see more than one message). This message was posted by the **HandleOrders** Azure Function App.
23. Leave the app running.
24. On the Start menu, click **Visual Studio 2017** to start another instance of Visual Studio 2017.
25. On the **File** menu, point to **Open**, and then click **Project/Solution**.
26. In the **Open Project** dialog box, go to **E:\\Labfiles\\Lab07\\Starter\\Exercise02\\Warehouse**, and then double-click **Warehouse.sln**. This app monitors the warehousequeue ServiceBus queue.
27. In Solution Explorer, click **App.config**.
28. On the **App.config** tab, in the **appSettings** section, replace **~CONNECTION STRING~** with the **Primary Connection String** value that you noted earlier for the **sb-20777-&lt;your name&gt;-&lt;the day&gt;** Service Bus.
29. Press F5 to build and run the application. You should see a messages appear that describe the items in the order you just placed. These messages were also posted by the **HandleOrders** Azure Function App.
30. On the **Adventure-Works** tab, place a few more orders. Notice how the details of the orders are processed by the HandleOrders function app, and the results displayed by the CustomerOrders and Warehouse apps.
31. When you have finished, close the Aventure-Works tab, but leave the **CustomerOrders**, and **Warehouse** apps running.
...
17. In the left panel, click **All resources**, and then click **20777-func-&lt;your initials and day&gt;**.
...
21. In the left panel, click **All resources**, and then click **20777-mod7-sql-&lt;your initials and day&gt;**.
22. On the **20777-mod7-sql-&lt;your initials and day&gt;** blade, under **MONITORING**, click **Metrics**.
...
1. In Internet Explorer, in the left panel, click **All resources**, and then click **20777a-databricks-&lt;your name&gt;-&lt;the day&gt;**.
2. In the **Message from webpage** dialog box, click **OK**.
3. On the **20777a-databricks-&lt;your name&gt;-&lt;the day&gt;** blade, click **Launch Workspace**.
4. On the **Azure Databricks** page, under **Common Tasks**, click **New Notebook**.
5. In the **Create Notebook** dialog box, in the **Name** box, type **rollingrevenue**.
6. In the **Language** drop-down list, click **Scala**.
7. In the **Cluster** drop-down list, click **&lt;your name&gt;-cluster**, and then click **Create**.
8. In the first cell of the notebook, type the following code:
...
6. Press F5 to build and run the application. You will see messages indicating that old orders are being deleted.
...
4. On the **IoT hub** blade, on the **Basics** tab, under **Resource Group**, click **Use existing** then, and then in the drop-down list, click **20777Mod07**.
...
1. In the left panel, click **All resources**, and then click **20777a-mod7-sql-&lt;your name&gt;-&lt;the day&gt;**.
...
1. In the left panel, click **All resources**, and then click **20777-func-&lt;your name&gt;-&lt;the day&gt;**.
2. On the **20777-func-&lt;your name&gt;-&lt;the day&gt;** blade, click **Start**, and wait until the **Status** is set to **Running**.
...
5. On the **New Function** blade, in the **Language** drop-down list, click **C#**.
6. In the **Name** box, type **ProcessDelivieries**.
7. Under **Azure Cosmos DB trigger**, accept the existing **Azure Cosmos DB account connection** configuration.
8. In the **Collection name** box, type **Deliveries**.
9. Select the **Create lease collection if it does not exist** check box.
10. In the **Database name** box, type **Adventure-Works**.
11. Leave **Collection name for leases** set to **leases**, and then click **Create**.
12. On the **run.csx** blade, click **20777-func-&lt;your name&gt;-&lt;the day&gt;**.
13. Under **Configured features**, click **Application settings**.
14. On the **Application settings** tab, in the **Application settings** section, click **+ Add new setting**.
15. In the **APP SETTING NAME** box, type **EndpointUrl**.
16. In the **VALUE** box, type the **URI** of your **20777a-mod7-sql-&lt;your name&gt;-&lt;the day&gt;** Cosmos DB account.
17. Click **+ Add new setting**.
18. In the **APP SETTING NAME** box, type **PrimaryKey**.
19. In the **VALUE** box, type the **primary key** of your **20777a-mod7-sql-&lt;your name&gt;-&lt;the day&gt;** Cosmos DB account.
20. Click **+ Add new setting**.
21. In the **APP SETTING NAME** box, type **Database**.
22. In the **VALUE** box, type **Adventure-Works**.
23. Click **+ Add new setting**.
24. In the **APP SETTING NAME** box, type **Collection**.
25. In the **VALUE** box, type **Data**, and then click **Save**.
26. In the left pane, click the **ProcessDeliveries** function.
27. On the **run.csx** blade, at the top of the file, under the reference to the **Microsoft.Azure.Documents.Client** assembly, add a reference to the **Newtonsoft.Json** assembly, as shown by the following code:
...
28. Add the following **Using** directives to those already in the script:
...
30. Modify the definition of the **Run** method to indicate that it supports asynchronous operations.
...
32. Above the **if** statement, add the following lines of code that retrieve the configuration settings required to connect to the Data collection in the **Adventure-Works** database, and then use these settings to create a DocumentClient object:
...
33. Inside the **if** statement, add the following statements:
...
34. Inside the **foreach** block, add the following code:
    ...
    log.Info(deliveryDoc.ToString());
    ```
35. Inside the **foreach** block, immediately after the code you have just entered, add the following statements:
...
36. Add the following code after the statements that you entered in the previous step. This code runs the query that fetches the corresponding product from the **Data** collection, and then updates the **QuantityInStock** field with the value from the new delivery. The **SaveDocument** function attempts to write the updated product document back to the **Data** collection. The **SaveDocument** returns a boolean that indicates whether the save operation was successful; while customers are placing orders, the stock levels are updated, and it is important not to lose these changes. The **SaveDocument** method uses ETags to prevent this situation from occuring. The *do* loop iterates, repeatedly fetching the latest version of the product document and attempting to save it until the operation is successful.
    ...
    do
    {
        ...
        log.Info($"product is {productDoc.ProductName}"); 
...
37. After the closing brace of the **Run** method, but above the definition of the **ProductDeliveryRecord** class, add the following function. This function saves the document, using the ETag to ensure that the document has not been modified by another user since it was retrieved.

    ```CSharp
    // Save a document and check for conflicts by using the ETag
    // Return true if the update was successful, false otherwise
    private static async Task<bool> SaveDocument(Document doc, DocumentClient client, TraceWriter log)
    {
        try
        {
            ...
            log.Info($"Document replaced. Status code is {response.StatusCode}");
            return true;
        }
        catch (DocumentClientException dce)
        {
            ...
            {
                log.Info("Update failed - another user already changed this document. Requery and try again");
            }
            return false;
        }
    }
