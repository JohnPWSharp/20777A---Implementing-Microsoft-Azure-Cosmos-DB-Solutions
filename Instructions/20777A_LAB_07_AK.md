...
5. On line 4, change the **resourceGroupLocation** variable to specify the name of your nearest location.
...
12. In the Azure portal, in the left panel, click **All resources**, and then click **20777-mod7-sql-\<*your initials\>\<day*\>**.
13. On the **20777-mod7-sql-\<*your initials\>\<day*\>** blade, under **Settings**, click **Keys**.
...
22. On the **App.config** tab, replace **\~URI\~** with the **URI**, and replace **\~KEY\~** with the **PRIMARY KEY** you noted earlier for the **20777-mod7-sql-\<*your initials\>\<day*\>** Cosmos DB account.
...
3. On the **Create Library** page,  under **Library Source**, click **Upload**, under **Library Type**, click **Jar**, in the **Library Name** box, type **Cosmos DB Connector**, and then click the text **Drop JAR here**.
4. In the **JAR File** box, go to the **E:\Labfiles\Lab07\Starter\Exercise01** folder, click **azure-cosmosdb-spark_2.3.0_2.11-1.2.0-uber.jar**, and then click **Open**.
5. When the JAR file has been uploaded, click **Create**.
6. On the **Cosms DB Connector** page, select the check box for the **\<*your name*\>-cluster** row, click **Install**  and wait until the status of the library changes to **Installed**.
7. In the toolbar on the left of the blade, click **Azure Databricks**.
8. Under **Common Tasks**, click **New Notebook**.
9. In the **Create Notebook** dialog box, in the **Name** box, type **monitorstocklevels**.
10. In the **Language** drop-down list, click **Scala**.
11. In the **Cluster** drop-down list, click **\<*your name*\>-cluster**, and then click **Create**.
12. In the first cell of the notebook, enter the following code.
...
14. In the new cell, type the following code. Replace **--URI--** with the **URI**, and replace **--KEY--** with the **PRIMARY KEY** values that you noted earlier for the **20777-mod7-sql-\<*your initials\>\<day*\>** Cosmos DB account.
...
26. In the new cell, type the following code. Replace **--URI--** with the **URI**, and replace **--KEY--** with the **PRIMARY KEY** values that you noted earlier for the **20777-mod7-sql-\<*your initials\>\<day*\>** Cosmos DB account.
...
5. On the **App.config** tab, replace **\~URI\~** with the **URI**, and replace **\~KEY\~** with the **PRIMARY KEY** values that you noted earlier for the **20777-mod7-sql-\<*your initials\>\<day*\>** Cosmos DB account.
...
8. In the toolbar at the top of the notebook, click **Run All**. Wait for the final cell in the notebook to indicate that it has started streaming data; the cell should run continuously, and you will see a **Last updated** message appear at regular intervals.
    ...
    > **Note**: The cell that starts streaming the data from the change feed might report the error **Failed to load class "org.slf4j.impl.StaticLoggerBinder"**. You can ignore this error.

9.  Switch to the Azure portal.
10. In Internet Explorer, in the left panel, click **All resources**, and then click **20777-mod7-sql-\<*your initials\>\<day*\>**.
11. On the **20777-mod7-sql-\<*your initials\>\<day*\>** blade, click **Data Explorer**, expand **Adventure-Works**, expand **ProductSales**, and then click **Documents**.
...
2. On the **20777-func-\<*your name\>-\<the day*\>** blade, under **Configured features**, click **+ New function**.
3. On the **CHOOSE A DEVELOPMENT ENVIRONMENT** page, click **In-portal**, and then click **Continue**.
4. On the **CREATE A FUNCTION** page, click **More templates**, and then click **Finish and view templates**.
5. On the **Choose a template below or go to the quickstart** blade, click **Cosmos DB trigger**. 
...
6. In the **Name** box, type **HandleOrders**.
7. Under **Azure Cosmos DB trigger**, click **new**.
8. In the **Connection** dialog box, click **Azure Cosmos DB account**.
9. In the **Subscription** drop-down list, select your own subscription.
10. In the **Database Account** drop-down list, click **20777-mod7-sql-\<*your initials\>\<day*\>**, and then click **Select**.
11. On the **New Function** blade, in the **Collection name** box, type **Data**.
12. Select the **Create lease collection if it does not exist** check box.
13. In the **Database name** box, type **Adventure-Works**.
14. Leave **Collection name for leases** set to **leases**, and then click **Create**.
15. In the **Function Apps** pane, expand **Function**, expand **HandleOrders**, and then click **Integrate**.
16. In the **Outputs** box, click **New Output**.
17. Click **Azure Service Bus**, and then click **Select**.
18. In the **Azure Service Bus output** box, under the message **This integration requires the following extensions. Microsoft.Azure.WebJobs.Extensions.ServiceBus**, click **Install**
19. In the **Message type** drop-down list, click **Service Bus Queue**.
20. In the **Message parameter name** box, type **customerOrdersQueue**.
21. Next to **Service Bus connection**, click **new**.
22. In the **Connection** dialog box, click **Service Bus**.
23. In the **Namespace** drop-down list, click **sb-20777-\<*your name\>-\<the day*\>**, and then click **Select**.
24. In the **Queue name** box, type **customerordersqueue**, and then click **Save**
25. Repeat steps 16 through 24 to add another Service Bus output with the following configuration:
...
26. In the **Function Apps** pane, click the **HandleOrders** function.
27. On the **run.csx** blade, type the following code to add a reference to the **Newtonsoft.Json** assembly at the top of the file, after the reference to the **Microsoft.Azure.DocumentDB.Core** assembly:

    ```CSharp
    #r "Newtonsoft.Json"
    ```

28. Add the following **Using** directive to the code, after the existing **Using** directives. This directive brings the **Newtonsoft.Json** namespace into scope:

    ```CSharp
    using Newtonsoft.Json;
    ```

29. After the closing brace of the **Run** function, add the following class definitions to the file:
...
30. Modify the definition of the **Run** method and add the two output parameters for the Service Bus queues, as follows:
...
31. Remove the following two statements from the body of the function:
...
33. Inside the **foreach** statement, add the following code:
...
34. Inside the nested **if** statement you added in the previous step, add the following code:
...
6.  On the **Web.config** tab, in the **appSettings** section, replace **\~URI\~** with the **URI**, and replace **\~KEY\~** with the **PRIMARY KEY** values that you noted earlier for the **20777-mod7-sql-\<*your initials\>\<day*\>** Cosmos DB account.
...
9.  On the **App.config** tab, in the **appSettings** section, replace **\~URI\~** with the **URI**, and replace **\~KEY\~** with the **PRIMARY KEY** values that you noted earlier for the **20777-mod7-sql-\<*your initials\>\<day*\>** Cosmos DB account.
...
16. In the left panel, click **All resources**.
17. In the **Message from webpage** dialog box, click **OK**.
18. On the **All resources** blade, click **20777-func-\<your initials and day\>**.
19. On the **20777-func-\<your initials and day\>** blade, click **Stop** to halt the function app.
...
1. In the Azure portal, in the left panel, click **All resources**.
2. In the **Message from webpage** dialog box, click **OK**.
3. On the **All resources** blade, click **20777a-databricks-\<*your name\>-\<the day*\>**.
4. On the **20777a-databricks-\<*your name\>-\<the day*\>** blade, click **Launch Workspace**.
5. On the **Azure Databricks** page, under **Common Tasks**, click **New Notebook**.
6. In the **Create Notebook** dialog box, in the **Name** box, type **rollingrevenue**.
7. In the **Language** drop-down list, click **Scala**, and then click **Create**.
8. On the **Detached** menu, click **\<*your name*\>-cluster**.
9. In the first cell of the notebook, type the following code:
...
11. In the new cell, type the following code. replace **\~URI\~** with the **URI**, and replace **\~KEY\~** with the **PRIMARY KEY** values that you noted earlier for the **20777-mod7-sql-\<*your initials\>\<day*\>** Cosmos DB account.
...
13. In the new cell, type the following code:
...
15. In the new cell, type the following code:
...
16. In the toolbar at the top right-hand side of the cell, on the **Edit Menu** menu, click **Add Cell Below**.
17. In the new cell, type the following code:
...
18. In the toolbar at the top right-hand side of the cell, on the **Edit Menu** menu, click **Add Cell Below**.
19. In the new cell, type the following code:
...
20. In the toolbar at the top right-hand side of the cell, on the **Edit Menu** menu, click **Add Cell Below**.
21. In the new cell, type the following code:
...
22. In the toolbar at the top right-hand side of the cell, on the **Edit Menu** menu, click **Add Cell Below**.
23. In the new cell, type the following code:
...
9. In the toolbar at the top of the notebook, click **Run All**.
10. If the **This notebook is not attached to a cluster** dialog box appears, click **Start and Run**.
11. Wait for the penultimate cell in the notebook to indicate that it has started streaming data; the cell should run continuously, and you will see a **Last updated** message appear at regular intervals.
12. Allow the notebook to run for another minute, and then in the toolbar at the top right-hand side of the final cell in the notebook, on the **Run Cell** menu, click **Run Cell**. A table displaying data for the last few minutes (1 row per minute) should appear. The figures in the **revenueperminute** and **revenueperminutebeforediscount** columns show the rolling revenue for the 5 minutes that end in the **time** column.
13. Below the table, click the graph drop-down arrow, and then click **Line (deprecated)**.
14. Click **Plot Options**.
15. In the **Customize Plot** dialog box, delete all columns from the **Keys**, **Series groupings**, and **Values** boxes.
16. In the **All fields** box, click the **time** column and drag it into the **Keys** box.
17. In the **All fields** box, click the **revenueperminute** column and drag it into the **Values** box.
18. In the **All fields** box, click the **revenueperminutebeforediscount** column and drag it into the **Values** box.
19. In the **All fields** box, click the **discounted** column and drag it into the **Values** box.
20. Click **Apply**. The table in the notebook should be replaced with a graph depicting the rolling revenue values.
21. In the toolbar at the top right-hand side of the cell, on the **Run Cell** menu, click **Run Cell**. The graph should be updated with the latest values. 
22. Wait for another minute and run the cell again. Repeat this process for 5 minutes. At this point, the data in the graph should start to level off. This is because for the first four minutes, the number of orders in the database is starting from a low point (no orders). After 5 minutes, the number of orders in the preceding 5 minute window should be reasonably constant, and the rolling revenue should also tend to even out.
23. In the toolbar at the top of the notebook, click **Stop Execution**.
24. Close the **Azure Databricks** portal.
25. In Visual Studio 2017, on the **Debug** menu, click **Stop Debugging**.
26. Close **Visual Studio 2017**.
...
4. On the Start menu, click **Visual Studio 2017**.
...
10. Return to the Azure portal, click **All resources**, and then click **20777-mod7-sql-\<*your initials\>\<day*\>**, and then click **Data Explorer**.
...
13. In the left panel, click **All resources**, and then click **20777-func-\<your name\>-\<the day\>**.
